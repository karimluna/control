...
\section{Transfer Functions and Block Algebra}\smallskip \hrule height 2pt \smallskip
    ...
    \begin{align*}
        a_0\frac{d^ny}{dt^n}+a_1\frac{d^{n-1}y}{dt^{n-1}}+\cdots+a_ny=b_0\frac{d^mu}{dt^m}+b_1\frac{d^{m-1}u}{dt^{m-1}}+\cdots+b_mu,
    \end{align*}
    where $n\ge m$ and $a_k,b_k\in\mathbf{R}$. To derive the transfer function $G(s)$, we apply the Laplace transform term by term assuming zero initial conditions ($u(t) \rightarrow U(s)$, $y(t) \rightarrow Y(s)$) and solve for the ratio of output to input:
    \begin{align*}
